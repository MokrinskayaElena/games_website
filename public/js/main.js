...
let currentRequiredScore = 0; 
let levels = [
  { countRows: 6, countCols: 7, gemSize: 80, requiredScore: 10, countMove: 11},
  { countRows: 8, countCols: 8, gemSize: 60, requiredScore: 20, countMove: 9},
  { countRows: 8, countCols: 15, gemSize: 60, requiredScore: 30, countMove: 11}
];
...
function initGame () {
    ...
    createLevelButtons(); // добавляем кнопки уровней
...
function createPage() {
    ...
    components.container.style.height = "80vh";
...
function createCursor () {
    ...
    components.cursor.style.border = "40px solid rgba(0, 0, 255, 0.46)";
    components.cursor.style.borderRadius = "40px";
    components.cursor.style.position = "absolute";
    components.cursor.style.display = "none";
    components.cursor.style.boxShadow = "10px 5px 10px rgba(0, 0, 255, 0.4)";
...
function updateMoves() {
    ...
    if (moveElement) {
        moveElement.innerHTML = config.countMove;
    }
}
function scoreInc ( count ) {
    ...
    if (config.countMove <= 0) {
        // Проверяем, достиг ли игрок необходимого счета
        if (config.countScore < currentRequiredScore) {
            isGameOver = true;
            setTimeout(function() {
                endLevel("Поражение! Не удалось набрать достаточное количество очков.");
            }, 1500);
            // return;
        }
    }
    // Проверка на достижение текущего уровня
    if (config.countScore >= currentRequiredScore) {
        isGameOver = true; // чтобы не повторять
        setTimeout(function() {
            endLevelWin("Поздравляем! Вы победили в уровне!");
...
function handlerTab ( event, target ) {
    if( target.classList.contains( config.gemClass ) && config.gameState ) {
...
// Функция для переключения уровня
function startLevel(levelIndex) {
    const level = levels[levelIndex];
    // обновляем параметры конфигурации
    config.countRows = level.countRows;
    config.countCols = level.countCols;
    config.gemSize = level.gemSize;

    // добавляем обновление countMove
    config.countMove = level.countMove;

     // обновляем текущий требуемый счет
    currentRequiredScore = level.requiredScore;
